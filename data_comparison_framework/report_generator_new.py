...
def generate_ydata_profile_reports(source_df: pd.DataFrame, target_df: pd.DataFrame, timestamp: str) -> dict:
    try:
        ...
        comparison_profile = source_df.profile_report(title="Comparison Data Profile")
...
def generate_datacompy_report(source_df: pd.DataFrame, target_df: pd.DataFrame, column_mapping: dict, join_keys: Optional[List[str]], timestamp: str) -> str:
    try:
        source_cols = list(column_mapping.keys())
        target_cols = [column_mapping[col] for col in source_cols]
        source_compare_df = source_df[source_cols].copy()
...
def generate_regression_report(source_df: pd.DataFrame, target_df: pd.DataFrame, column_mapping: dict, output_path: str) -> None:
    try:
        source_cols = list(column_mapping.keys())
        target_cols = [column_mapping[col] for col in source_cols]
        source_compare_df = source_df[source_cols].copy()
...
def generate_side_by_side_report(source_df: pd.DataFrame, target_df: pd.DataFrame, column_mapping: dict, join_columns: Optional[List[str]], output_path: str) -> bool:
    try:
        source_cols = list(column_mapping.keys())
        target_cols = [column_mapping[col] for col in source_cols]
        source_compare_df = source_df[source_cols].copy()
